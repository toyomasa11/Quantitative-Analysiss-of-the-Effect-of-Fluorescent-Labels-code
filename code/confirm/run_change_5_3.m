...
M51 = readmatrix(filename,'Sheet','51'); % 5400s data
M52 = readmatrix(filename,'Sheet','52'); % 5400s data
M31 = readmatrix(filename,'Sheet','31'); % 5400s data
M32 = readmatrix(filename,'Sheet','32'); % 5400s data
...
%tunefig('qiita', fig1);         % Edit only one figure
tunefig('qiita', [fig1, fig2]);   % edit multiple figures at once
...
% Type: axes
ax_st(2) = ax_st(1);    % axis settings copied once from document
ax_st(2).LineWidth = 1.5;   % Change only line width to 1.5
% Type: line
ln_st(2) = ln_st(1);    % line settings copied from document
ln_st(2).LineWidth = 1.5;   % Change only line width to 1.5

% ======= qiita =======
% Type: figure
fig_st(3).Color = 'w';%'none';  % BackgroundColor = transparent 'none'/white 'w'
% Type: axes
ax_st(3) = ax_st(1);    % axis settings copied once from document
ax_st(3).LineWidth = 2;   % Change to line width only
ax_st(3).FontSize= 15;
% Type: line
ln_st(3) = ln_st(1);    % line setting is copied from document
ln_st(3).LineWidth = 2;   % Change only line width
