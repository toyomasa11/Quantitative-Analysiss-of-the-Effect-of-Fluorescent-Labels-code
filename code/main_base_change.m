...
r1 = optimvar('r1',1,"LowerBound",0,"UpperBound",1e-1); % create optimization variables
...
for ex = 1 %<=================================
    ...
    % ==================    
    for i=1:3 %<=================================
        y0 = [100 100 0 0]; % initial state
        myfcn = fcn2optimexpr(@RtoODE_rev,r,str{ex,2}(:,i),y0);
...
for ex=1%<=================================
    for i=1:3%<=================================
        [t,y] = ode15s(@(t,y)diffun_rev_infinite(t,y,r_),str{ex,2}(:,i),y0,opts); % simple model
