...
% (1) Designate the input time (120s) 
tim_in = 120; 

% (2) Designate the data positions 
dd = [3 4 5];  % main data 
...
% =========================================================================
% 1st data:DNA reversible (5') 
% =========================================================================

ex=1;
fileID =M31;
dele = 7; % First line to be deleted
[t_str,y_str] = data_processing(flag,fileID,tim_in,dd,pc,nc,1,3,2,ex); % method 2 Data Correction
 % data_processing(flag?,Excel sheet name, input time ,dd,pc,nc,1,Maximum number of graphs, which method to use) 
t_str(dele:end, :) = [];
y_str(dele:end, :) = []; % Eliminate unwanted time in t 202 ⇒ 1010 sec. and after
...
for ex=1
    ...
    j=1;
    t_inter1 = [min(str{ex,2}(:,j)):1:max(str{ex,2}(:,j))]'; % time
    y_inter1 = interp1(str{ex,2}(:,j),str{ex,3}(:,j),t_inter1); % Extraction of y values corresponding to time
    ...
    clear t_ave y_ave y_std % Array Preparation
